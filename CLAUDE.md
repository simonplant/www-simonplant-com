...
npm run build     # Type-check (astro check), build to dist/, then Pagefind index
npm run preview   # Preview production build locally
```

**Search:** Pagefind full-text search lives at `/search`. Its index and UI
bundle are generated by the `pagefind --site dist` step of `npm run build`, so
search only works under `npm run preview` and in production — **not** `npm run
dev` (the page degrades to a "run a build" message there). Indexing is scoped
to `data-pagefind-body` on the `<article>` of content detail pages, so nav,
footer, and listing pages don't pollute results.
